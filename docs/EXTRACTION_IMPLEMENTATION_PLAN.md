...
*Last updated: September 21, 2026 (rewritten after the extraction/dedup planning session — see §0).*
...
| Error handling | Unspecified | **Two tiers + one per-post rescue + a run-level Gemini breaker** (§5.2–5.3) | One bug or one Gemini outage must not abort or hammer the whole run |
...
**This plan owns:** the `events` and `extractions` tables and models; the stage-based dispatch refactor in `AccountPipeline`; `ObjectStore#get`; `GeminiClient`; the prompt, response schema, parser and gate; `Extractor`; the run-level Gemini breaker; the per-post rescue; the monitoring hooks (§10); the eval set.

**The dedup plan owns (later):** the stage enum swap and `PostLoader` guard flip; `DHash` and `images.dhash` population; the `event_groups` table (one row per real-world event) and `dedup_decisions` table (pair scores, audit log only); adding `events.event_group_id` as a nullable FK; the `Deduplicator`; its pipeline wiring. A post with no matches becomes a group of one; changing a dedup threshold means clearing `event_group_id`, deleting groups, and recomputing, with no post state to migrate.
...
**QR codes** on a poster are a hint that the post is a sign-up-bearing event poster. The model does not guess where a QR points.
...
def process_post(post, raw)
  ...
  Extractor.call(post) if post.media_processed? && !breaker.open?
...
### 5.3 Run-level Gemini breaker (in memory, one run)

`IngestionRunner` creates one `GeminiBreaker` and passes it into every `AccountPipeline.call`. It counts **consecutive whole-service failures** (`error_kind: "whole_service"`: bad key, quota/429, timeout, 5xx). At **5** in a row it opens: the rest of the run skips `Extractor`, scraping and media continue, and affected posts wait at `media_processed` for the next cron pass. Any other outcome resets the count — a success, or a this-post failure (Gemini answered, so the service is up). It is not persisted; it resets every run. It is the run-scoped cousin of Phase 4's per-source circuit breaker, which needs DB memory across runs.

**Opening posts to `#clubsync-alerts`** (Q10 resolved): when the breaker opens, `DiscordNotifier` also posts a breaker-open line to `#clubsync-alerts` (a new hermetic method — same rescue-and-swallow contract as `post_run_summary`). Rationale: a persistent Gemini outage otherwise looks like a healthy run — the run finishes, `stage_results` shows posts stuck at `media_processed`, and only `#clubsync-log` (skim-only) carries the fact. Exact message shape is the health plan's concern; the decision is made.
...
4. **Parse** (`ExtractionParser`, pure) and **gate** (`ConfidenceGate`, pure). Invalid response → failed (this-post). A structurally valid but semantically impossible date (e.g. `2026-02-30`) or JSON truncated at the token limit both count as `InvalidResponseError` — the response schema guarantees shape, not truth.
5. **Derive** `is_event` from the category via `Categories`.
6. **Success — one short transaction:** create the succeeded `extractions` row; create the `events` row if `is_event`; update the post (`category`, `is_event`, `stage: :extracted`, clear `last_error`/`stage_failed_at`).
7. **Failure:** write a failed `extractions` row and set `last_error`/`stage_failed_at` on the post; `stage` stays `media_processed`.
...
- **Model:** a Gemini vision-capable model via Google AI Studio, ID in `GEMINI_MODEL`. **Working default (2026-09-21): Gemini 3.8 Flash.** **Verify current model IDs, free-tier limits, structured-output support, and image token cost for 10-image carousels at build time**, including which JSON-schema keywords the chosen model actually supports; the default stands unless Pass S shows cheaper/faster Flash-Lite-class tier handles the 10-image carousels and structured output. One model, no tiering, low temperature.
...
1. **Event test and precedence** exactly as in §3, with the sign-up rules.
...
8. **QR codes:** a hint only; never guess where they point.
...
1. **`ConfidenceGate` (pure, rule-based)** runs regardless of what the LLM reports:
...
(Add `AND e.event_group_id IS NULL` — or whatever the dedup plan's representative-row logic turns out to be — once the dedup plan adds `event_groups`.)
...
| `ExtractionPrompt` | Prompt text + JSON response schema + `VERSION` (bumped by hand; stored in `extractions.prompt_version`) |
...
| `ConfidenceGate` | **Pure** rule checks (§8) |
| `Extractor` | The stage service (§6): guard, orchestration, DB writes, `Result` |
| `GeminiBreaker` | In-memory consecutive-failure counter (§5.3) |
...
- Pure: `ExtractionParser`, `ConfidenceGate`, `Categories`, `GeminiBreaker`.
...
- Dispatch: a stalled `media_processed` post **is** retried (regression for §0); an unexpected error on one post doesn't stop the others; the breaker opens at 5 and resets on success.
...
- [x] Add `Extractor` to dispatch; create `GeminiBreaker` in `IngestionRunner` and pass it down
...
Dedup is paused until this phase has produced real `events` rows. What we know so far:
...
- The session also owns: the stage enum swap, the `PostLoader` guard flip, `event_groups`, `dedup_decisions`, and `events.event_group_id`. If dedup ends up as query-time grouping, the `deduped` enum value is simply removed.
...
| 9 | Throttling / per-run cap on Gemini calls | **Resolved:** none beyond the breaker — no `GEMINI_MIN_INTERVAL_SECONDS`/`EXTRACTION_MAX_PER_RUN` knobs in v1; sequential ≤410/run on a 2-day cadence fits free-tier RPM; knobs only if Pass S timing says otherwise |
| 10 | Should the breaker opening post to `#clubsync-alerts`? | **Resolved:** yes — hermetic breaker-open line to `#clubsync-alerts` (§5.3); message shape owned by the health plan |
...
| 13 | Exact Gemini model ID and free-tier limits | **Working default: Gemini 3.8 Flash** (§7), verified at Pass S (model IDs, structured-output support, image-token cost for 10-image carousels); Flash-Lite-class tier reconsidered only if it clears those checks |
