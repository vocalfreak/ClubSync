...
class ConfidenceGateTest < ActiveSupport::TestCase
  ...
  test "drops too-short and too-long venues" do
    [ "X", "a" * 201 ].each do |venue|
      attrs = gated(build(:gemini_payload).merge("venue" => venue))
  ...
  test "keeps ends on or after starts" do
    ...
    assert_equal Date.new(2026, 2, 20), gated(payload)[:ends_date]
  end
  ...
  test "forces starts_at confidence to zero only outside the sanity window" do
    ...
    recent = gated(payload_with.call("2026-03-10"))
    assert_equal 0.8, recent[:confidence][:starts_at], "same-day event keeps its confidence"

    inside = gated(payload_with.call("2026-03-24"))
    assert_equal 0.8, inside[:confidence][:starts_at]

    too_early = gated(payload_with.call("2026-02-20"), posted_at: posted_at)
    ...
    too_late = gated(payload_with.call("2027-04-01"), posted_at: posted_at)
    assert_equal 0.0, too_late[:confidence][:starts_at], "beyond 12 months after posted_at"

    boundary = gated(payload_with.call("2027-03-10"), posted_at: posted_at)
